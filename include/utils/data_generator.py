...
class RealisticSalesDataGenerator:
    def __init__(self, start_date="2021-01-01", end_date="2021-12-31"):
        ...
        self.stores = {
            ...
            "store_008": {'location': 'San Diego', 'size': 'medium', 'base_traffic': 500},
    ...
    def get_day_of_week_factor(self, date: pd.Timestamp) -> float:
        ...
        dow_factors = [0.9, 0.95, 1.0, 1.05, 1.1, 1.2, 1.15]
        return dow_factors[dow]    
        
    def generate_promotions(self) -> pd.DataFrame:
        """Generate promotional events data."""
        promotions = []
        
        major_events = [
            ('Black Friday', 11, 4, 5, 0.25), #november, 4th friday, duration 5 days, 25% discount``
            ('Cyber Monday', 11, 4, 2, 0.20), #november,   4th monday, duration 3 days, 20% discount
            ('Christmas Sale', 12, 25, 7, 0.30), #december, 25th, duration 7 days, 30% discount
            ('New Year Slee', 1, 1, 7, 0.20), #january, 1st, duration 7 days, 20% discount
            ('Liberation Day Sale', 4, 30, 5, 0.15), #april, 30th, duration 5 days, 15% discount
            ('labor Day Sale', 5, 1, 5, 0.15), #may, 1st, duration 5 days, 15% discount
            ('Independence Day Sale', 9, 2, 5, 0.20), #september, 2nd, duration 5 days, 20% discount
            ('Halloween Sale', 10, 31, 5, 0.25) #october, 31st, duration 5 days, 25% discount
        ]        
        ...
        while current_date <= self.end_date:
            ...
            for event_name, month, day, duration, discount in major_events:
                if event_name == 'Black Friday':
                    ...
                    end_november = pd.Timestamp(year, 11, 30)
                    fridays = pd.date_range(november, end_november, freq='W-FRI')
                    if len(fridays) >= 4:
                        event_date = fridays[3]
                    else:
                        # If for some reason we don't have 4 Fridays, skip this year
                        continue
                else:
                    try:
    ...
    def generate_sales_data(self, output_dir: str = 'tmp/sales_data/') -> Dict[str, List[str]]:
        ...
        while current_date <= self.end_date:
            ...
            logger.info(f"Generating sales data for date: {date_str}")
            ...
            for store_id, store_info in self.stores.items():
                ...
                weather_impact = np.random.normal(loc=1.0, scale=0.1)
                weather_factor = max(0.5, min(1.2, weather_impact)) #clamp between 0.5 and 1.2
                ...
                #calculate store customer traffic
                store_traffic = int(base_traffic * dow_factor * holiday_factor * weather_factor * store_event_impact
                                    * np.random.normal(loc=1.0, scale=0.05)) #5% noise
                ...
                for product_id, product_info in self.all_products.items():
                    #product seasonality
                    seasonality_factor = self.get_seasonality_factor(current_date, product_info['seasonality'])
                    ...
                    if not promotions_df.empty:
                        ...
                        if not promo.empty:
                            ...
                            #promotions increase sales by discount percent
                    ...
                    base_quantity = store_traffic * 0.01 * size_factor * price_factor
                    
                    quantity = int(
                        base_quantity * seasonality_factor * promotion_factor *
                        np.random.normal(loc=1.0, scale=0.2) #20%
                    )
            ...
            #inventory data
            if daily_inventory_data and current_date.dayofweek == 6: #only weekly on Sundays
