...
#include the utils module from the include directory
sys.path.append("/usr/local/airflow/include")

# Defer heavy imports to inside task functions to speed up DAG parsing
...
@dag(
    ...
    tags=['ml', 'training', 'sales', 'sales_forecast'],
)
...
def sales_forecast_training(): 
    ...
    def extract_data_task():
        ...
        for kind, paths in file_paths.items():
            print(f"Generated {len(paths)} {kind} files")
